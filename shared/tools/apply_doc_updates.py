...
def apply_suggestions(
    ...
    *,
    base_branch: str = "",
    branch_prefix: str = "docs/update-for",
) -> dict:
    ...
    Args:
        ...
        pr_number: The source PR number (optional for non-PR mode).
        repo: "owner/repo" string (informational only).
        base_branch: If supplied, create the docs branch from this branch
            instead of deriving it from a PR head ref.  Used by the
            docs_generator which operates outside of a PR context.
        branch_prefix: Prefix for the docs branch name.  Defaults to
            "docs/update".
    ...
    # ------------------------------------------------------------------
    if base_branch:
        # Non-PR mode: use the supplied branch directly.
        head_ref = base_branch
        try:
            # For local backend, just get the current HEAD SHA.
            head_sha = backend.read_file(".git/HEAD", ref=base_branch)
        except Exception:
            # If that doesn't work (e.g., API backend), try PR head ref
            # as fallback, or use "HEAD" as a placeholder.
            head_sha = "HEAD"
    else:
        try:
    ...
    # ------------------------------------------------------------------
    suffix = f"-pr-{pr_number}" if pr_number else f"-{int(time.time())}"
    ...
    for idx, suggestion in enumerate(suggestions):
        ...
        try:
            backend.write_file(
                path=doc_path,
                content=new_content,
                message=f"docs: update {doc_path}" + (f" for PR #{pr_number}" if pr_number else ""),
                branch=branch_name,
            )
            commit_count += 1
    ...
    try:
        ...
        if pr_number:
            ...
            pr_title = "docs: generate documentation"
            body_lines = [
