...
class GitHubAPIBackend:
    ...
    def list_files(
        ...
        """List files via the GitHub Git Trees API (recursive).

        Uses ``GET /repos/{owner}/{repo}/git/trees/{ref}?recursive=1``
        to fetch the full tree in a single call, then filters locally.
        """
        resp = github_get(
            f"/repos/{self._repo_path}/git/trees/HEAD",
            params={"recursive": "1"},
        )
        data = resp.json()
